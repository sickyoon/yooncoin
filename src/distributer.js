...
class Distributer {
  ...
  distribute = async () => {
    ...
    await this.instances.erc20.methods
      .approve(this.instances.router.options.address, MaxUint256)
      .send({
    ...
    // console.log(
    //   "* transfer 300 erc20 from accounts[0] to accounts[1] as accounts[1]"
    // );
    // await this.instances.erc20.methods
    //   .transferFrom(this.accounts[0], this.accounts[1], 300)
    //   .send({
    ...
    // console.log(
    //   "* transfer 300 weth from accounts[0] to accounts[1] using accounts[1]"
    // );
    // await this.instances.weth.methods
    //   .transferFrom(this.accounts[0], this.accounts[1], 300)
    //   .send({
  ...
  printInstance = async (instance) => {
    console.log(
      `* ${await instance.methods
        .name()
        .call()} (${await instance.methods.symbol().call()})`
    );
    console.log(
      `  ${pad("total")} ${await instance.methods.totalSupply().call()}`
    );
    console.log(
      `  ${pad("acc0")} ${await instance.methods
        .balanceOf(this.accounts[0])
        .call()} (${await instance.methods
        .allowance(this.accounts[0], this.instances.router.options.address)
        .call()})`
    );
    console.log(
      `  ${pad("acc1")} ${await instance.methods
        .balanceOf(this.accounts[1])
        .call()} (${await instance.methods
